...
const ContactForm = () => {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "company" && value.length > 50) {
      ...
      setCharLimitErrors((prev) => ({
        ...prev,
        company: "Maximale Zeichen erreicht!",
      }));
      return;
    }
    ...
    if (name === "name" && value.length > 100) {
      setSnackbarMessage(
        "Maximale Zeichenbegrenzung für Vor- & Nachname erreicht."
      );
      setOpenSnackbar(true);
      setCharLimitErrors((prev) => ({
        ...prev,
        name: "Maximale Zeichen erreicht!",
      }));
      return;
    }
    ...
    if (name === "email" && value.length > 100) {
      ...
      setCharLimitErrors((prev) => ({
        ...prev,
        email: "Maximale Zeichen erreicht!",
      }));
      return;
    }
    ...
    if (name === "message" && value.length > 500) {
      ...
      setCharLimitErrors((prev) => ({
        ...prev,
        message: "Maximale Zeichen erreicht!",
      }));
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSnackbarMessage(
        "Fehler beim Senden der E-Mail. Bitte versuche es später noch einmal."
      );
      setOpenSnackbar(true);
    } finally {
      setIsSubmitting(false);
  ...
  return (
    ...
    >
      ...
      {errors.queryType && (
        <FormHelperText error>{errors.queryType}</FormHelperText>
      )}
      ...
      <FormControl fullWidth>
        <InputLabel
          ...
          sx={{
            backgroundColor: colors.white,
            color: formData.gender ? colors.gray : colors.gray,
            padding: "0 5px",
          }}
        >
          Anrede *
      ...
      <FormControl fullWidth sx={{ marginTop: "10px" }}>
        <label>
          <input
            ...
            required
          />
          Ich willige in die Verarbeitung meiner personenbezogenen Daten zum
          Zwecke der Kontaktaufnahme bzgl. der Angebotserstellung ein. Sie haben
          das Recht, Ihre Einwilligung jederzeit zu widerrufen. Durch den
